...
export default function StudioMobile({ songs: propSongs = [] }: Props) {
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (!lastId) return;
      const lastSong = songs.find(s => s.id === lastId);
      if (lastSong) {
        setSelected(lastSong);
        setScreen('record');
      }
    } catch {}
  }, [songs]);
